...
Implemented and committed (db6290e, 2026-08-24). Board entry was stale
("Not started") relative to git history; corrected 2026-08-25.

### Changes made

- Recorded ADR-002 (PGlite worker-thread pattern, driven by the Windows
  WASM-initdb constraint), ADR-003 (MQTT bridging/re-encryption design), and
  ADR-004 (current multi-device support stance) — each documenting an
  already-implemented decision grounded in the actual code.
- All three ADRs are marked `Status: Proposed, awaiting review`, corrected
  from an initial `Accepted` marking — accepting a decision record isn't the
  implementer's or orchestrator's call.
- ADR-004 surfaces an open question rather than resolving it: the
  persistence layer is fully device-scoped and tested that way, while
  `MqttGateway`'s `mqtt_nodes` table and its distance-anchor heuristic are
  not.

### Validation performed

Documentation only, no code change; no automated validation applicable.

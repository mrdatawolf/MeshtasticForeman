...
Implemented and committed (8edf578, 2026-08-24). Board entry was stale
("Not started") relative to git history; corrected 2026-08-25.

### Changes made

- `client.ts` now calls `clearDbLock(DATA_DIR)` immediately before
  `openDb(DATA_DIR)`, mirroring the terrain-cache scripts' existing pattern
  exactly.

### Validation performed

- Full daemon suite: 156/156 passing.
- Manual reproduction (kill -9 the daemon, restart) did **not** actually
  reproduce the blocking failure against the pinned
  `@electric-sql/pglite@0.4.6` — `openDb()` succeeded immediately even
  without this fix, in two independent trials.

### Unresolved risks

- The underlying failure mode this task assumed could not be confirmed in
  this environment/version. The fix is kept as a cheap, safe precaution
  matching established precedent (the terrain-cache scripts), but its actual
  necessity against the currently pinned PGlite version is unconfirmed.
